...
func NewDatastore(_ context.Context, cfg mongodriver.MongoDriverConfig) (m *Mongo, err error) {
	...
	databaseCollectionBuilder := map[mongohealth.Database][]mongohealth.Collection{
		mongohealth.Database(m.Database): {mongohealth.Collection(m.ActualCollectionName(config.RecipesCollection))}}
...
func (m *Mongo) GetRecipes(ctx context.Context, offset int, limit int) (*models.RecipeResults, error) {
	...
	totalCount, err := m.connection.Collection(m.ActualCollectionName(config.RecipesCollection)).Find(ctx, bson.D{}, &recipeItems,
...
func (m *Mongo) GetRecipe(ctx context.Context, id string) (*models.Recipe, error) {
	...
	err := m.connection.Collection(m.ActualCollectionName(config.RecipesCollection)).FindOne(ctx, bson.M{"_id": id}, &recipe)
	if err != nil {
...
func (m *Mongo) AddRecipe(ctx context.Context, item models.Recipe) error {
	_, err := m.connection.Collection(m.ActualCollectionName(config.RecipesCollection)).UpsertById(ctx, item.ID, bson.M{"$set": item})
...
func (m *Mongo) UpdateAllRecipe(ctx context.Context, id string, update bson.M) (err error) {
	_, err = m.connection.Collection(m.ActualCollectionName(config.RecipesCollection)).Must().UpdateById(ctx, id, update)
	if err != nil {
		if errors.Is(err, mongodriver.ErrNoDocumentFound) {
