...
func NewDatastore(_ context.Context, cfg mongodriver.MongoConnectionConfig) (m *Mongo, err error) {
	...
	databaseCollectionBuilder[(mongohealth.Database)(m.Database)] = []mongohealth.Collection{recipesCollection}
...
func (m *Mongo) GetRecipes(ctx context.Context, offset int, limit int) (*models.RecipeResults, error) {
	var (
		recipeItems []*models.Recipe
		totalCount  int
		err         error
	)

	totalCount, err = m.connection.Collection(recipesCollection).Find(ctx, bson.D{}, &recipeItems,
		mongodriver.Sort(bson.M{"_id": 1}), mongodriver.Offset(offset), mongodriver.Limit(limit))
	if err != nil {
		log.Error(ctx, "error finding recipes", err)
		return nil, err
	}
...
func (m *Mongo) GetRecipe(ctx context.Context, id string) (*models.Recipe, error) {
	...
	err := m.connection.Collection(recipesCollection).FindOne(ctx, bson.M{"_id": id}, &recipe)
	if err != nil {
...
func (m *Mongo) AddRecipe(ctx context.Context, item models.Recipe) error {
	_, err := m.connection.Collection(recipesCollection).UpsertById(ctx, item.ID, bson.M{"$set": item})
...
func (m *Mongo) UpdateAllRecipe(ctx context.Context, id string, update bson.M) (err error) {
	_, err = m.connection.Collection(recipesCollection).Must().UpdateById(ctx, id, update)
	if err != nil {
		if errors.Is(err, mongodriver.ErrNoDocumentFound) {
